...
class WebsiteRouteTests(TestCase):
    ...
    def test_personal_services_page_contains_single_h1_breadcrumb_and_service_content(self):
        response = self.client.get(reverse("personal"))
        content = response.content.decode()
        main_content = content.split("<main", 1)[1].split("</main>", 1)[0]

        self.assertContains(
            response,
            '<span class="personal-services-hero__title-line">面向企业主、投资人与家庭</span>',
            html=False,
        )
        self.assertContains(
            response,
            '<span class="personal-services-hero__title-line">提供法国本地服务</span>',
            html=False,
        )
        ...
        self.assertNotIn(reverse("personal_cross_border_tax_risk"), main_content)
        self.assertNotIn("查看详情 ↗", main_content)
        self.assertIn(reverse("contact"), content)
    ...
    def test_retired_personal_service_routes_permanently_redirect_to_personal_overview(self):
        route_names = [
        ...
        for route_name in route_names:
            with self.subTest(route_name=route_name):
                ...
                self.assertEqual(response.url, reverse("personal"))
    ...
    def test_cookie_policy_page_contains_current_cookie_facts(self):
        ...
        self.assertEqual(content.count("<h1"), 1)
        self.assertIn("<title>Cookie 与类似技术说明｜Acoeurs Consulting</title>", content)
        self.assertContains(response, "csrftoken")
        self.assertContains(response, "sessionid")
        self.assertContains(response, "当前网站未启用 Google Analytics 或 GA4")
        self.assertContains(response, "其他广告及营销追踪")
        self.assertContains(response, "当前网站未使用访问分析、广告、营销或其他非必要追踪技术")
        self.assertContains(response, "当前未设置针对非必要 Cookie 的专门 Cookie consent Banner")
        self.assertContains(response, "如果网站未来引入访问分析、广告、嵌入式视频、地图、聊天或其他第三方追踪技术")
        self.assertContains(response, "我们将重新评估并相应更新 Cookie 说明、隐私政策、用户告知和同意机制")
    ...
    def test_privacy_policy_page_contains_current_processing_and_boundaries(self):
        ...
        self.assertContains(response, "个人信息处理者")
        self.assertContains(response, "艾克斯（杭州）咨询服务有限公司")
        self.assertContains(response, "91330110MAC40RHUXB")
        ...
        self.assertContains(response, "网站托管服务商")
        self.assertContains(response, "企业邮件服务商")
        self.assertContains(response, "中国香港")
        self.assertContains(response, "境外处理与个人信息出境")
        self.assertContains(response, "当前联系表单内容不写入 Contact 或 Inquiry 数据库，不进入 CRM")
        self.assertContains(response, "网站不进行自动客户画像或自动化决策")
        self.assertContains(response, "当前网站不会自动将联系表单内容发送给律师、会计、银行、保险或其他合作方")
        self.assertContains(response, "知情权、决定权")
        self.assertContains(response, "限制或拒绝处理")
        self.assertContains(response, "查阅、复制、更正、补充或删除个人信息")
        self.assertContains(response, "撤回基于同意的处理")
        self.assertContains(response, "解释个人信息处理规则")
        self.assertContains(response, "投诉与反馈")
    ...
    def test_legal_notice_page_contains_current_operator_and_service_boundaries(self):
        response = self.client.get(reverse("legal"))
        content = response.content.decode()
        ...
        self.assertContains(response, "艾克斯（杭州）咨询服务有限公司")
        self.assertContains(response, "91330110MAC40RHUXB")
        self.assertContains(response, "浙江省杭州市余杭区仓前街道文一西路1217号11幢806室")
        self.assertContains(response, "Acoeurs Consulting / 艾克斯咨询")
        self.assertContains(response, "Alibaba Cloud")
        self.assertContains(response, "中国香港")
        self.assertContains(response, "当前网站不提供在线交易、在线支付或前台用户账户")
        self.assertContains(response, "受监管工作由具备相应资格的专业人士或相关机构完成")
        self.assertContains(response, "不构成对具体审批、交易、贷款、税务处理或其他受监管事项结果的保证")
        ...
        for href in hrefs:
            if href.startswith("/") and not href.startswith("/static/"):
                with self.subTest(href=href):
                    linked_response = self.client.get(href, follow=True)
    ...
    def test_service_detail_pages_are_accessible_from_overview_pages(self):
        ...
        personal_main_content = personal_content.split("<main", 1)[1].split("</main>", 1)[0]
        ...
        for route_name in [
            ...
            with self.subTest(route_name=route_name):
                self.assertNotIn(reverse(route_name), personal_main_content)
